...
class DBpdosqlite {
	...
	public function __construct($filename) {
        ...
        $this->link = new \PDO("sqlite:".$this->filename);
	...
	function sendQuery($str){
		...
		$str = str_replace("bigint NOT NULL AUTO_INCREMENT PRIMARY KEY", " integer PRIMARY KEY AUTOINCREMENT ", $str);
		$str = str_replace("BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY", " integer PRIMARY KEY AUTOINCREMENT ", $str);
		$str = str_replace("ENGINE=InnoDB DEFAULT CHARSET=utf8", " ", $str);
	...
	function query($str) {
        ...
        try {
			...
			throw new \InvalidArgumentException('SQL-Error: '.$str, 10);
        }
		$d = microtime(true)-$T;
...
#vd($data);vd($table);exit;
	...
	function lastID() {
		// {{{
		#$lastId = SQLiteDatabase::lastInsertRowid();
...
class DBDebug extends DBpdosqlite {
	// {{{
	function query($str){
		$T0 = microtime(true);
