...
class DB {
	...
	public function __construct($host, $user, $pass, $name, $characterset="utf8") {
		...
		$this->res = mysql_select_db($this->name, $this->link );
	...
	function fetch(){
		...
		return $data = $this->data = mysql_fetch_assoc($this->res);
	}
	function fetchAll(){
		$data = array();
		if($this->res===false || $this->res===true) {$this->logError($this->lastQuery);return $data;}
	...
	function DB_error($DB){
		if (!$DB->res){
			return die('Ungueltige Abfrage: ' . mysql_error());
		}
	}
